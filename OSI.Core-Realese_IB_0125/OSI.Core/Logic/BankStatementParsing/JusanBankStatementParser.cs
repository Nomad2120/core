...
{
    public class JusanBankStatementParser : IStatementParser
    {
        ...
        {
            if (lines.Count < 4 || !lines[3].StartsWith("ДатаНачала"))
                throw new Exception("Ошибка при проверке даты начала выписки: Неверный формат файла");

            if (!DateTime.TryParse(lines[3].Split('=')[1], out DateTime dt))
        ...
        {
            if (lines.Count < 5 || !lines[4].StartsWith("ДатаКонца"))
