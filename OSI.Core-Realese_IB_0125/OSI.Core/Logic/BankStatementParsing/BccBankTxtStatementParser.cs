...
{
    public class BccBankTxtStatementParser : IStatementParser
    {
        ...
        {
            if (lines.Count < 7 || !lines[6].StartsWith("ДатаНачала"))
                throw new Exception("Ошибка при проверке даты начала выписки: Неверный формат файла");

            if (!DateTime.TryParse(lines[6].Split('=')[1], out DateTime dt))
        ...
        {
            if (lines.Count < 8 || !lines[7].StartsWith("ДатаКонца"))
                throw new Exception("Ошибка при проверке даты конца выписки: Неверный формат файла");

            if (!DateTime.TryParse(lines[7].Split('=')[1], out DateTime dt))
