...
namespace OSI.Core.Helpers
{
    public static class RusNumber
    {
            ...
            bool putTenge)
        {
            ...
            decimal ostatok = value < 1 ? value : Math.Abs(value) % Math.Truncate(value);
            if (putTenge)
            {
                if (less100 != 0)
                    str.AppendWithSpace("тенге");
                else
                    str.Append("тенге");
            }
            if (ostatok > 0)
                str.AppendWithSpace(ostatok.ToString(NumberFormatInfo.InvariantInfo).Replace("0.", ""));
            else if (putTenge || less100 != 0)
                str.AppendWithSpace("00");
            else
                str.Append("00");
            if (putTenge)
                str.AppendWithSpace("тиын");

            return str.ToString();
        }

        private static string RusSpelledOut(
            this decimal value,
            bool male)
        {
            if (value >= 1000000000000000)
